...
import { getTypeProducts, filterProduct } from "../../actions/product";
...
const TypeProducts = ({
  getTypeProducts,
  auth,
  product: { productTypes, typeProducts, loading, filterData },
  match,
  getLikedProducts,
  filterProduct,
}) => {
  ...
  const getTypeName = () => {
    for (let i = 0; i < productTypes.length; i++) {
      for (let j = 0; j < productTypes[i].type.length; j++) {
        if (productTypes[i].type[j].id === parseInt(match.params.id)) {
          setTypeName(productTypes[i].type[j].type_name);
          break;
        }
      }
      if (typeName !== "") break;
  ...
  useEffect(() => {
    getTypeProducts(match.params.id);
  }, [getTypeProducts, match.params.id]);

  useEffect(() => getTypeName());
