...
import { getTypeGroupProducts, filterProduct } from "../../actions/product";
...
const TypeGroupProducts = ({
  getTypeGroupProducts,
  auth,
  product: { productTypes, typeGroupProducts, loading, filterData },
  match,
  getLikedProducts,
  filterProduct,
}) => {
  ...
  const [typeGroupName, setTypeGroupName] = useState("");
  ...
  const getTypeGroupName = () => {
    for (let i = 0; i < productTypes.length; i++) {
      if (productTypes[i].id === parseInt(match.params.id)) {
        setTypeGroupName(productTypes[i].name);
        break;
      }
    }
  };
  ...
  useEffect(() => {
    getTypeGroupProducts(match.params.id);
  }, [getTypeGroupProducts, match.params.id]);

  useEffect(() => getTypeGroupName());
